...
async def main() -> None:
    ...
    backend = resolve_graph_backend(repo, default="sqlite")
    golden = golden_from_sqlite(repo / ".victor" / "project.db", args.targets, MIN_CALLER_FILES)
    ...
    store = create_graph_store("auto", project_path=repo)
    await store.initialize()

    # `mode` participates in the query-cache key. Varying it by direction keeps
    # a prior run's answer from being served to this one — a cached result is
    # indistinguishable from a fast one, and that mistake has been made here.
    config = RetrievalConfig(
        seed_count=5,
        max_hops=2,
        top_k=args.k,
        mode=f"eval-{args.direction}",
    ...
    for target in golden:
        ...
        started = time.perf_counter()
        try:
            result = await retriever.retrieve(f"who calls {target['target']}", config)
