...
authors:
  - name: Vijaykumar Singh
    ...
    github: vjsingh1984
reviewers: []
discussion: https://github.com/anvai-labs/victor/discussions/0015
---
...
- [x] Delete the `__getattr__` shim and the `_FEP_0015_DEPRECATED` map from `step_handlers.py`
      (also drop the now-unused `import warnings` if nothing else uses it).
- [x] Tighten the guard test (`test_step_handlers_exports.py`): replace the "old name warns
...
- [x] Re-grep `victor/ tests/ verticals/` for the old names immediately before merging; expect
