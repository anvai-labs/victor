...
- **Phase 3b — surfaces + batches (in progress).** API surface **✅ landed**: `/chat` returns a
  ...
  structurally blocks `chat`-group subcommands-with-args.) Still deferred: a CLI arming *flag* (needs
  the CLI to first expose the tool-approval enable/ask-on-tools flags); TUI paused lane; multi-tool
  batch partiality; streaming resume; chained pauses.
