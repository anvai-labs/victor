...
| TD-11 | ProximaDB CCG Backend | `ProximaGraphStore` and the embedded provider are implemented behind the per-repo flag with SQLite still default. Remaining GA work is a buildable ProximaDB embedded wheel, live parity/bench evidence, service-mode closure, and default graduation. See `docs/architecture/proximadb-codegraph-backend.md`. | Medium | In Progress | `victor/storage/` |
