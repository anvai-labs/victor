...
| TD-14 | Orchestrator Regrowth | `victor/agent/orchestrator.py` is 4,690 lines — it regrew ~34% after TD-R1 declared it resolved at 3,510. The facade pattern is intact (delegation to services is real), but the file is a god-object again. Decompose; ratchet guard landed 2026-07-02 (`tests/unit/runtime/test_hotspot_size_guard.py`) so it cannot silently regrow a third time — lower the caps as decomposition proceeds. **ADR-019 increment 1 (2026-07-30):** extracted the pure task-report metadata builders to `victor/agent/task_report_metadata.py` (now directly unit-tested); orchestrator 4,690→4,600, cap ratcheted to 4600. A deeper read showed the broader "metrics cluster" is state-coupling glue, not a clean facade — later increments target the genuinely-cohesive clusters (tool-strategy, conversation) per ADR-019. **ADR-019 increment 2 (2026-07-30):** extracted the tool-supply policy (Tool-Necessity Gate keyword sets + `_tool_skip_mode` + `_demote_tools_to_fit` budgeter) to `victor/agent/tool_supply_policy.py` (pure, callable-injected, directly unit-tested); orchestrator 4,600→4,402, cap ratcheted to 4402. Remaining tool-strategy methods reach `_tool_service` (a later ToolStrategyCoordinator increment). | High | In Progress | `victor/agent/` |
